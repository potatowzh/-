...
class Face_Recognizer:
    def __init__(self, detector, nn4_small2):
        ...
        detector = cv2.dnn.readNetFromCaffe(
            "F:\\old\\Intelligent-Elderly-Care-master\\Intelligent-Elderly-Care-master\\Data\\data_opencv\\deploy.prototxt.txt",
        ...
        self.nn4_small2.load_weights(
            'F:\\old\\Intelligent-Elderly-Care-master\\Intelligent-Elderly-Care-master\\Data\\weights\\nn4.small2.v1.h5')
    ...
    def get_face_database(self):
        if self.loaded:
            ...
            if os.path.exists("F:\\old\\Intelligent-Elderly-Care-master\\Intelligent-Elderly-Care-master\\Data\\data_faces_from_camera\\"):
                self.metadata = facenet.load_metadata("F:\\old\\Intelligent-Elderly-Care-master\\Intelligent-Elderly-Care-master\\Data\\data_faces_from_camera\\")
                self.name_known_cnt = 0
                ...
                for i in range(len(self.name_known_list)):
                    ...
                    self.id_known_list.append(
                        requests.get("http://127.0.0.1:8000/api/person/" + str(type) + "/" + str(
                            ...
                            if min(e_distance_list) < 0.58:
                                ...
                                if self.type_camera_list[k] == 'elder':
                                    ...
                                    if mode == 'happy':
                                        ...
                                        cv2.putText(img_with_name, 'happy', (startX + 5, startY - 45), cv2.FONT_ITALIC,
                                                    1,
                                                    (255, 255, 255), 1, cv2.LINE_AA)
                                        time_snap = datetime.now()
                                        cv2.imwrite('smile_detection' + str(time_snap).replace(':', '') + '.jpg',
                                                    img_with_name)
                                ...
                                img_with_name = self.draw_name(img_rd)
                                time_snap = datetime.now()
                                cv2.imwrite('stranger_detection' + str(time_snap).replace(':', '') + '.jpg',
                                            img_with_name)
                                if (datetime.now() - self.pre).total_seconds() > 5:
                                    t = threading.Thread(
                                        target=post(event=2,
                                                    imagePath='stranger_detection' + str(time_snap).replace(':',
                                                                                                            '') + '.jpg'))
                                    t.setDaemon(False)
                                    t.start()
                                    self.pre = datetime.now()
